...
| Path | Role |
|---|---|
| `src/game/` | Pure core: `types.ts`, `rules.ts` (line slide/merge, `canMove`, `hasWon`), `board.ts` (`createGame`, `move`, `continueAfterWin`), `spawn.ts` (spawn odds; always 2 or 4, with the share of 4s climbing from 10% to a 25% cap once the largest tile passes `WIN_VALUE`), `rng.ts` |
