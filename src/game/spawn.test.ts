...
describe('spawnOdds', () => {
  it('spawns only 2s and 4s at 90/10 before the first rung', () => {
    for (const highest of [0, 2, 256, 511]) {
  ...
  it('keeps every distribution a probability distribution', () => {
    for (const highest of [0, 512, 2048, 8192]) {
  ...
  it('makes small spawns rarer as the board grows', () => {
    const smalls = [0, 512, 2048, 8192].map((highest) => probabilityOf(spawnOdds(highest), 2));
...
describe('spawnValue', () => {
  const odds = spawnOdds(2048); // 2: 0.792, 4: 0.088, 8: 0.08, 16: 0.04
